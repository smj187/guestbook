...
const setMode = (newMode: "text" | "draw") => {
    mode.value = newMode;
};
</script>
...
<template>
    ...
    <div ref="contentRef" class="absolute inset-0 flex items-center flex-col w-full h-full pt-16 lg:pt-36">
        <div class="p-6 flex">
            <button @click="() => setMode('text')" :class="mode === 'text'
            ...
            <button @click="() => setMode('draw')" :class="mode === 'draw'
                    ? 'text-[hsl(0,0%,97%)] bg-[#92B648] hover:bg-[hsl(80,43%,47%)] border-[hsl(80,43%,48%)]'
                    : 'text-[#6E5932] bg-[#DEDCCE] hover:bg-[hsl(53,20%,89%)] border-[hsl(39,18%,79%)]'
                "
